...
double get_wall_time(void)
{
  ...
  if (!QueryPerformanceFrequency(&freq)) 
    raise_runtime_error( "Error getting clock frequency." );
  if (!QueryPerformanceCounter(&time))
    raise_runtime_error( "Error getting wall time." );
  ...
  struct timeval tm;
  if (gettimeofday( &tm, 0 )) raise_runtime_error( "Error getting wall time." );
